...
import model.mallet.topicmodel.JointSentimentPropagationTopicModelCopy;
import model.mallet.topicmodel.LDACompute;
...
public class FeatureCombinerAndWriter {
	 ...
	 */
	public static HashMap<Integer,List<Double>> CombineAllFeatures(List<String> contents) throws IOException, ParseException{
		featureMap.putAll(WordFeatures.GetFeatures(contents));
	...
	public static List<String> FormatFeaturesForSVM(HashMap<Integer,List<Double>> features,boolean Write2File,int label) throws IOException{
		...
		if(Write2File){
			WriterUtils.write2file(svmFeatures, "FeaturesForSVM.txt");
		}
		return svmFeatures;
	}
...
//		System.out.println(filter.size());
		if(Write2File){
			WriterUtils.write2file(wekaFeatures, "FeaturesForWeka.arff");
		}
		return wekaFeatures;
	}
...
//					continue;
		...
		WriterUtils.write2file(csvFeatures, "FeaturesForWeka.csv");
	}
